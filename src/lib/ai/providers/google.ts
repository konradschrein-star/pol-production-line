...
export class GoogleProvider implements AIProvider {
  ...
  async analyzeScriptSceneBasedWithDuration(
    ...
    console.log(`\n🎬 [Google Scene-Based] Starting scene-based analysis...`);
    ...
    console.log(`✅ [Google Scene-Based] Complete: ${result.scenes.length} total scenes`);
  ...
  async analyzeScriptSceneBased(
    ...
    try {
      ...
      console.log(`✅ [Google Scene-Based] Generated ${validated.scenes.length} scene(s)`);

      return validated;
    } catch (error: unknown) {
      const errorMessage = error instanceof Error ? error.message : 'Unknown error';
      console.error('❌ [Google Scene-Based] Analysis failed:', error);
      throw new Error(`Google scene-based analysis failed: ${errorMessage}`);
    }
  }
}
